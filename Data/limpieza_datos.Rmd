...
filtrar_df <- function(df) {
  ...
  #Se filtra el df por los países que son de nuestro interes. En este caso, limítrofes a Argentina. Dejamos de lado Brasil ya que su lengua no es el español (tal como le pedimos a la api). 
  df <- df %>% filter(location %like% "Argentina" | location %like% "Bolivia" |location %like% "Chile" | location %like% "Paraguay" |  location %like% "Uruguay")
